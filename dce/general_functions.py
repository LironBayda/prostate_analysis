...
def get_sigma(images,mask):
    ...
    sigma = np.asarray([np.std(image[mask >= 1].flatten()) for image in images])
    return np.asarray(sigma)
def get_C3_from_mask(path,img_name,mask):
    
    img=nib.load(join(path,img_name)).get_fdata()
    mask=get_mask(path, mask)
    temp=img[mask >= 1]
    c_p = np.sort(temp[temp >0])
    return c_p[-int(len(c_p)*0.25)]
